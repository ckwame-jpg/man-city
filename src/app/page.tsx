...
export default function Home() {
  ...
  return (
    <div className={`font-sans ${darkMode ? 'dark' : ''}`} style={{ fontFamily: "'Calibre', 'Inter', 'San Francisco', 'SF Pro Text', -apple-system, system-ui, sans-serif" }}>
      {/* Mobile Navbar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white dark:bg-[#011128] border-b border-black/10 dark:border-white/10 px-6 py-4 flex items-center justify-between">
        <span className="text-lg font-semibold text-black dark:text-[#F5F5F5]">Chris Prempeh</span>
        <div className="flex items-center space-x-4">
          <a href="https://github.com/ckwame-jpg" target="_blank" rel="noopener noreferrer" aria-label="GitHub">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-black dark:text-[#F5F5F5] hover:text-black dark:hover:text-white" viewBox="0 0 24 24" fill="currentColor">
          ...
          <button
            ...
            className="text-black dark:text-[#F5F5F5] text-sm border border-black/20 dark:border-white/20 px-2 py-1 rounded hover:bg-black/10 dark:hover:bg-[#F5F5F5]/10 transition"
          >
            {darkMode ? 'Light' : 'Dark'}
          </button>
          <button
          ...
          >
            <span className={`h-0.5 w-full bg-black dark:bg-[#F5F5F5] transform transition duration-300 ease-in-out ${menuOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
            <span className={`h-0.5 w-full bg-black dark:bg-[#F5F5F5] transition duration-300 ease-in-out ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`h-0.5 w-full bg-black dark:bg-[#F5F5F5] transform transition duration-300 ease-in-out ${menuOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
          </button>
        </div>
      </div>
      ...
      <button
        onClick={() => setDarkMode(!darkMode)}
        className="hidden md:block fixed top-6 right-8 z-50 text-black dark:text-[#F5F5F5] border border-black/20 dark:border-white/20 px-3 py-1.5 rounded-lg bg-white/80 dark:bg-[#011128]/80 hover:bg-black/10 dark:hover:bg-[#F5F5F5]/10 shadow transition"
        aria-label="Toggle Light/Dark Mode"
      >
        {darkMode ? 'Light' : 'Dark'}
      </button>
      {menuOpen && (
        <div className="md:hidden fixed top-16 left-0 right-0 bg-white dark:bg-[#011128] z-40 border-t border-black/10 dark:border-white/10 px-6 py-6 space-y-4 text-black dark:text-[#F5F5F5] text-base">
          <a
            href="#about"
            onClick={(e) => {
              e.preventDefault();
      ...
      <main
        ref={mainRef}
        className="min-h-screen px-6 md:px-28 grid grid-cols-1 md:grid-cols-[320px_1fr] gap-52 pt-36 md:pt-20 bg-white dark:bg-[#011128] text-black dark:text-[#F5F5F5] transition-colors"
      >
        <div className="hidden md:flex flex-col justify-between sticky top-20 self-start pl-4 pb-20 h-[calc(100vh-9rem)]">
          <div className="flex flex-col items-start justify-start flex-grow pt-0 space-y-1">
            <h1 className="text-6xl font-bold text-black dark:text-[#F5F5F5]">Chris Prempeh</h1>
            <h2 className="text-xl text-black/80 dark:text-[#F5F5F5]/80 mt-2">Technical Manager</h2>
            <p className="text-black/60 dark:text-[#F5F5F5]/60 mt-4 text-base mb-22">Future developer currently building backend systems that are reliable, clean, and built to scale.</p>
            <div className="space-y-6 text-base text-black/60 dark:text-[#F5F5F5]/60 font-medium">
              {[
                ...
                <div key={id} className="group flex items-center space-x-2">
                  <span
                    className={`h-px w-6 transition-all duration-300 ${
                      activeSection === id
                        ? 'bg-[#575365] w-8 dark:bg-[#F5F5F5]'
                        : 'bg-[#575365]/30 dark:bg-[#F5F5F5]/30 group-hover:bg-[#575365] dark:group-hover:bg-[#F5F5F5]'
                    }`}
                  />
                  <a
                    href={`#${id}`}
                    ...
                    className={`transition-transform duration-300 hover:text-black dark:hover:text-white hover:scale-105 ${
                      activeSection === id ? 'text-black dark:text-white scale-105' : ''
                    }`}
                  >
                    {label}
                  </a>
          ...
          <div className="flex space-x-4 mt-auto">
            <a href="https://github.com/ckwame-jpg" target="_blank" rel="noopener noreferrer" aria-label="GitHub">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-black dark:text-[#F5F5F5] hover:text-black dark:hover:text-white transition-transform hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
                <path fillRule="evenodd" clipRule="evenodd" d="M12 0C5.37 0 0 5.373 0 12a12 12 0 008.207 11.385c.6.11.82-.26.82-.577v-2.065c-3.338.727-4.043-1.61-4.043-1.61-.546-1.387-1.334-1.756-1.334-1.756-1.09-.745.083-.73.083-.73 1.205.084 1.84 1.238 1.84 1.238 1.072 1.835 2.812 1.305 3.498.998.108-.777.42-1.305.76-1.604-2.665-.305-5.466-1.334-5.466-5.933 0-1.31.468-2.381 1.235-3.22-.123-.305-.535-1.524.117-3.177 0 0 1.007-.322 3.3 1.23a11.49 11.49 0 016.003 0c2.292-1.552 3.297-1.23 3.297-1.23.653 1.653.24 2.872.118 3.177.77.839 1.234 1.91 1.234 3.22 0 4.61-2.803 5.625-5.476 5.921.432.37.814 1.103.814 2.222v3.293c0 .32.22.694.827.576A12.005 12.005 0 0024 12c0-6.627-5.373-12-12-12z"/>
              </svg>
            </a>
            <a href="https://www.linkedin.com/in/chris-prempeh/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-black dark:text-[#F5F5F5] hover:text-black dark:hover:text-white transition-transform hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 0h-14C2.24 0 0 2.24 0 5v14c0 2.76 2.24 5 5 5h14c2.76 0 5-2.24 5-5V5c0-2.76-2.24-5-5-5zM7.19 19H4.28V8.99h2.91V19zm-1.45-11.6c-.93 0-1.69-.76-1.69-1.7s.76-1.7 1.69-1.7 1.69.76 1.69 1.7-.76 1.7-1.69 1.7zM20 19h-2.91v-4.89c0-1.17-.02-2.67-1.63-2.67-1.63 0-1.88 1.27-1.88 2.58V19h-2.91V8.99h2.79v1.37h.04c.39-.74 1.33-1.52 2.74-1.52 2.93 0 3.47 1.93 3.47 4.44V19z"/>
              </svg>
            </a>
        ...
        <div className="space-y-32">
          <section id="about" className="scroll-mt-16 space-y-4 text-base sm:text-lg text-black/80 dark:text-[#F5F5F5]/80">
            <p>
          ...
          <section id="projects" className="scroll-mt-16 border-t border-black/10 dark:border-[#363946]/50 pt-10">
            <h2 className="text-2xl font-semibold text-black dark:text-[#F5F5F5] mb-5">projects</h2>
            <motion.div
              className="rounded-lg px-8 py-6 border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 backdrop-blur-md"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, ease: 'easeOut' }}
              viewport={{ once: true }}
            >
              <h3 className="text-2xl font-bold text-black dark:text-[#F5F5F5]">habit tracker</h3>
              <p className="dark:text-[#F5F5F5]/80 text-black/80 mt-2 text-base">
              ...
              <a
                ...
                className="inline-block mt-4 text-black dark:text-[#F5F5F5] hover:text-black dark:hover:text-white transition-transform hover:scale-110"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub Project Repository"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 transition-all duration-300 hover:scale-110 hover:text-black dark:hover:text-white" viewBox="0 0 24 24" fill="currentColor">
          ...
          <section id="experience" ref={experienceRef} className="scroll-mt-16 border-t border-black/10 dark:border-[#363946]/50 pt-10 pb-60">
            <h2 className="text-2xl font-semibold text-black dark:text-[#F5F5F5] mb-6">experience</h2>
            <div className="space-y-10 sm:ml-0 leading-loose">
              <div className="grid sm:grid-cols-[150px_1fr] sm:gap-10 items-start">
                <p className="text-base text-black/60 dark:text-[#F5F5F5]/60 mb-1 sm:mb-0">Oct 2024 — Present</p>
                <div>
                  <h3 className="text-xl font-semibold text-black dark:text-[#F5F5F5]">VAR Technical Manager · Sportec Solutions</h3>
                  <p className="text-black/80 dark:text-[#F5F5F5]/80 text-base mt-1">
              ...
              <div className="grid sm:grid-cols-[150px_1fr] sm:gap-10 items-start">
                <p className="text-base text-black/60 dark:text-[#F5F5F5]/60 mb-1 sm:mb-0">2021 — 2023</p>
                <div>
                  <h3 className="text-xl font-semibold text-black dark:text-[#F5F5F5]">Data Entry Operator · Amazon</h3>
                  <p className="text-black/80 dark:text-[#F5F5F5]/80 text-base mt-1">
          ...
          <div className="text-base text-black/50 dark:text-[#F5F5F5]/50 mt-12 mb-35 md:pl-4">
            <p>
