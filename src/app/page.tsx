        ...
        <div className="space-y-32">
          ...
          <section id="projects" className="scroll-mt-16 border-t border-[var(--border-subtle)] pt-10">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h3 className="text-2xl font-bold text-[var(--text-primary)]">gh-stats</h3>
              <p className="text-[var(--text-secondary)] mt-2 text-base">
                A CLI tool that pulls GitHub profile data and shows it right in your terminal. View user profiles, repo stats, recent activity, or compare two users side by side. Built with Click and Rich, installable via pip, and caches API responses so it stays fast.
              </p>
              <div className="flex flex-wrap gap-2 mt-4">
                {['Python', 'Click', 'Rich', 'GitHub API', 'httpx', 'pytest'].map((tag) => (
                  <span key={tag} className="px-3 py-1 text-xs rounded-full border border-[var(--border-tag)] bg-[var(--card-bg)] text-[var(--tag-text)]">
                    {tag}
                  </span>
                ))}
              ...
              <div className="flex items-center space-x-4 mt-4">
                <a
                  href="https://github.com/ckwame-jpg/gh-stats"
                  className="inline-block text-[var(--text-primary)] hover:opacity-80 transition-transform hover:scale-110"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="GitHub Project Repository"
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h3 className="text-2xl font-bold text-[var(--text-primary)]">sports etl</h3>
              <p className="text-[var(--text-secondary)] mt-2 text-base">
                An ETL pipeline that pulls NFL player stats from the ESPN API, calculates PPR fantasy scores, and loads everything into PostgreSQL. Has upsert logic so it handles reruns cleanly, a CLI for triggering runs and viewing top players, and tracks pipeline run history.
              </p>
              <div className="flex flex-wrap gap-2 mt-4">
                {['Python', 'pandas', 'PostgreSQL', 'SQLAlchemy', 'ESPN API', 'Docker'].map((tag) => (
                  <span key={tag} className="px-3 py-1 text-xs rounded-full border border-[var(--border-tag)] bg-[var(--card-bg)] text-[var(--tag-text)]">
                    {tag}
                  </span>
                ))}
              ...
              <div className="flex items-center space-x-4 mt-4">
                <a
                  href="https://github.com/ckwame-jpg/sports-etl"
                  className="inline-block text-[var(--text-primary)] hover:opacity-80 transition-transform hover:scale-110"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="GitHub Project Repository"
